...
void ScenarioManager::processModuleSpecificCommand(cXMLElement *node)
{
    ...
    if (!scriptable)
        error("<%s> not understood: it is not a built-in command of %s, and module class %s "  //TODO be more specific
                        "is not scriptable (does not subclass from IScriptable) at %s", node->getTagName(), getClassName(), mod->getClassName(), node->getSourceLocation());
...
void ScenarioManager::processCreateModuleCommand(cXMLElement *node)
{
    ...
    cModule *parentModule = cSimulation::getActiveSimulation()->getSystemModule()->getModuleByPath(parentModulePath);
...
void ScenarioManager::processDeleteModuleCommand(cXMLElement *node)
{
    ...
    cModule *module = cSimulation::getActiveSimulation()->getSystemModule()->getModuleByPath(modulePath);
    if (module == nullptr)
...
void ScenarioManager::processConnectCommand(cXMLElement *node)
{
    ...
    if (srcMod->getParentModule() != destMod->getParentModule())
        throw cRuntimeError("The parent modules of src-module and dest-module are differ at %s", node->getSourceLocation());
    ...
    srcGate = isSrcGateInOut ? srcMod->gateHalf(srcGateName.c_str(), cGate::OUTPUT, srcGateIndex) : srcMod->gate(srcGateName.c_str(), srcGateIndex);
    destGate = isDestGateInOut ? destMod->gateHalf(destGateName.c_str(), cGate::INPUT, destGateIndex) : destMod->gate(destGateName.c_str(), destGateIndex);
